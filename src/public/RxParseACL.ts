...
export class RxParseACL {
        ...
        if (arg.length > 0) {
            arg.forEach(currentItem => {
                if (currentItem instanceof RxParseUser) {
                    ...
                    throw new TypeError('ParseACL constructor need RxParseUser or RxParseRole.');
                }
            });
        } else {
    ...
    private _getAccess(
        ...
        if (userId instanceof RxParseUser) {
            userId = userId.objectId;
            if (!userId) {
                throw new Error('Cannot get access for a RxParseUser without an ID');
            }
        } else if (userId instanceof RxParseRole) {
            const name = userId.name;
            if (!name) {
    ...
    public getRoleReadAccess(role: RxParseRole | string): boolean {
        ...
        if (typeof role !== 'string') {
            throw new TypeError(
                'role must be a RxParseRole or a String'
            );
        }
